export const getAnalyzingCodeQuery = (code, question) => {
  return `Hãy nhìn vào đoạn code dưới đây '${code}' và đây là câu hỏi '${question}',
   Hãy phân tích một cách ngắn gọn điểm sai và điểm tốt ở trong đoạn code
   , đừng đưa ra đáp án, chỉ cần nêu ra cách cải thiện, 
   Chỉ trả lời bằng format object {good:string, bad:string, improvement: string}
   , trong đó good là những điểm tốt ở trong code, bad là những lỗi sai hoặc điểm chưa
    tốt trong code, improvement là gợi ý những cách để cải thiện bài code, 
    không có thêm chữ cái nào trong câu trả lời ngoại trừ object đó`;
};
...
export const getAskingChatGptQuery = (query) => {
  return `Hãy phân tích chủ đề '${query}' thật sâu và đưa ra code ví dụ minh họa nếu có. 
  Chỉ trả lời bằng format object '{explanation:string, examples:array}',
  examples là array có các string là các đoạn code ví dụ tương tự như này ' examples: [
